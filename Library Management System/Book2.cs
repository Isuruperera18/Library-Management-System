...
namespace Library_Management_System
{
	public partial class Book2 : Form
	{
		...
		{
			if (txtCoNumber.Text == "" || txtCoISBN.Text == "" || txtCoTitle.Text == "" || txtCoLibraryname.Text == "" || txtCoYear.Text == "")
			{
				MessageBox.Show("Enter Required Fields");
			}
			else
			{
				...
				{
					con.Open();
					cmd = new SqlCommand("insert into CopyRecords (Copy_Number,ISBN,Title,Library_Name,Copy_Year) values ('" + txtCoNumber.Text + "','" + txtCoISBN.Text + "','" + txtCoTitle.Text + "','" + txtCoLibraryname.Text + "' ,'" + txtCoYear.Text + "')", con);
					cmd.ExecuteNonQuery();
					con.Close();
					MessageBox.Show("Data has been saved");
					clear();
